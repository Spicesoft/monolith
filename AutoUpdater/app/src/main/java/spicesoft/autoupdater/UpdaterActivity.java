...
/*
    ...
    public void enableAutoUpdate(View view) {
        manager = (AlarmManager)getSystemService(Context.ALARM_SERVICE);
        int interval = 60000; // 1 minute
        manager.setRepeating(AlarmManager.RTC_WAKEUP, System.currentTimeMillis(), interval, pendingIntent);
        Toast.makeText(this, "Automatic update enabled", Toast.LENGTH_SHORT).show();
    }

    public void disableAutoUpdate(View view) {
        if (manager != null) {
            manager.cancel(pendingIntent);
            Toast.makeText(this, "Automatic update disabled ", Toast.LENGTH_SHORT).show();
        }
    }

    public void updateNow(View view){
        UpdateDownloader ud = new UpdateDownloader();
        ud.setContext(getApplicationContext());
